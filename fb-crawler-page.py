...
def parse_htmltext(下載目錄路徑,粉絲專頁id):
	...
	body = soup.find('body')
	feed_articles = body.select('div[role="feed"]')[1].select('div[role="article"]')
	...
	for article in articles:
		try:
			...
			貼文連結 = re.findall('<a aria-label="加強推廣貼文".[^>]*href=".[^"]*target_id=(.[0-9]*).[^"]*"', str(article))
			...
			if len(貼文連結) == 0 :
				...
				貼文連結 = str(貼文連結[0])
				拼接連結 = "https://www.facebook.com/permalink.php?story_fbid="+貼文連結+"&id="+粉絲專頁id
				print("拼接連結 : " + 拼接連結)
			if len(貼文內容) == 0 :
				print("沒找到貼文內容")
				check = False
			...
			if check == True :
				...
				col = col + 1
				sheet.write(row, col, xlwt.Formula('HYPERLINK("' + 拼接連結 + '";"' + 貼文內容 + '")'))
				col = col + 1
